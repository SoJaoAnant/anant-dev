...
export type ProjectCardProps = {
  /** PNG (or any) image in /public or remote (if configured) */
  imageSrc: string;
  imageAlt?: string;
  ...
  color1?: string;
  color2?: string;
  /** Where the image should navigate */
  href: string;
};
...
export default function ProjectCard({
  ...
  return (
    <article className="group relative flex w-full max-w-5xl gap-6 rounded-2xl border border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.02] p-5 backdrop-blur-sm">
      ...
      >
        <div className="flex items-center justify-center aspect-[16/9] w-full p-4">
          <Image
            src={imageSrc}
            alt={imageAlt}
            width={320}
        ...
        <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
          <div className="absolute inset-0 bg-gradient-to-tr from-fuchsia-500/20 via-transparent to-white/10" />
      ...
      <div className="min-w-0 flex-1">
        <h3 className="text-2xl font-semibold tracking-wide text-white/90">{title}</h3>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {stack.map((s) => (
            <span
              key={s}
              className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs tracking-wide text-white/70"
            >
              {s}
            </span>
          ))}
